...
type (
	tbUserFileModel interface {
		...
		SaveFile(ctx context.Context, data *SaveData)(sql.Result, error)
	}
...
func (m *defaultTbUserFileModel) SaveFile(ctx context.Context, data *SaveData) (sql.Result, error) {
	query := fmt.Sprintf("insert into %s (user_name,user_email,file_name,file_size,file_sha1,type) values (?, ?, ?, ?, ?,?)", m.table)
	ret, err := m.conn.ExecCtx(ctx, query, data.UserName, data.UserEmail, data.FileName, data.FileSize, data.ShareIdentity,data.Type)
	return ret, err
}
